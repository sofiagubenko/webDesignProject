...
function addMarkers(cityFilter, dateFilter) {
    ...
    filteredExhibitions.forEach((exhibition)=>{
        L.marker([
            exhibition.lat,
            exhibition.lon
        ]).addTo(map).bindPopup(`<b>${exhibition.name}</b><br>${exhibition.city}<br>${exhibition.date}`);
    });
    // Show a message if no exhibitions are found
    const messageElement = document.getElementById('message');
    if (filteredExhibitions.length === 0) messageElement.textContent = 'No exhibitions found for the selected filters.';
...
//# sourceMappingURL=map.de0548b7.js.map
